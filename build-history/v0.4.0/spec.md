...
The skill follows the Agent Skills specification. `SKILL.md` carries frontmatter (`name`, a third-person `description` with trigger words, `license`, `compatibility`, `metadata.version`) and a short body. Detail lives in `references/`, one level deep:

```
skill/
  SKILL.md
  references/
    cli.md          generated from the CLI's own help
    format.md
    logging.md
    predicting.md
    grading.md
...
`references/cli.md` is generated by `pnpm generate:skill-cli`. A test fails when it is behind the code. `SKILL.md` lists every command in one line and points at `cli.md` for options and examples.

`ath init` always writes the skill to `.agents/skills/athletic-standard/`, and into any of `.claude`, `.cursor`, `.codex`, `.gemini`, `.github` that already exist. `--no-skill` skips it.

```
ath skill            each installed copy, its version, and whether it is current
ath skill install    refresh every copy
```

Every other command warns once on stderr when an installed copy is older than the shipped one. Nothing is said when no copy exists. `--json` runs, `init`, and `skill` never warn.
